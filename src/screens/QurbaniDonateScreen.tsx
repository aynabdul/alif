...
const QurbaniDonateScreen = () => {
  ...
  const fetchQurbanis = async () => {
    try {
      setLoading(true);
      setError(null);
      console.log('Fetching qurbanis for donate screen...');
      const response = await qurbaniService.getQurbanis();
      console.log('Qurbani donate screen response:', response);
      
      if (response.success && response.data) {
        console.log('Filtering qurbanis for donation:', response.data);
        ...
        const donateQurbanis = response.data.filter(
          qurbani => (qurbani.showinwhichpage === 'donatequrbani' || qurbani.showinwhichpage === 'donate') && 
                    (!qurbani.countrySelection || qurbani.countrySelection === userCountry)
        );
        console.log(`Found donation qurbanis for ${userCountry}:`, donateQurbanis.length);
        setQurbanis(donateQurbanis);
      } else {
        console.error('API returned success=false or no data:', response);
        setError(`Failed to fetch donation options: ${response.message || 'Unknown error'}`);
      }
      setLoading(false);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Unknown error';
      console.error('Error fetching donations:', err);
      setError(`Failed to load donation options: ${errorMessage}`);
      setLoading(false);
    }
  };
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      {error ? (
        ...
        <SectionList
          ...
          ListHeaderComponent={
            <>
              <Text style={[styles.title, { color: theme.colors.text }]}>Donate Qurbani</Text>
              <GuideCard />
            </>
          }
